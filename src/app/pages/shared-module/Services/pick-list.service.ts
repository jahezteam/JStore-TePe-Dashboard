...
export class PickListService {
  ...
  getgroups() {
    return this.caller.GetWithFullUrl(
      this.apiUrl + '/ImageGroup/GetGroupsLookup',
    );
  }
  getTypes() {
  ...
  getPermissions() {
    return this.caller.GetWithFullUrl(
      this.apiUrl + '/Role/GetPermissionsLookup',
    );
  }
  getRoles() {
  ...
  getUserRoles(id: any) {
    return this.caller.GetByIdQuesry(
      this.apiUrl + '/User/UerRoles?UserId=' + id,
    );
  }

  getQuestionLookups() {
    return this.caller.GetWithFullUrl(
      this.apiUrl + '/QuestionBank/GetQuestionBankLookups',
    );
  }
  getPackageLookups() {
    return this.caller.GetWithFullUrl(
      this.apiUrl + '/Product/GetProductLookups',
    );
  }
  getPackageLookupsForCoupons(couponId: number, type: number) {
    return this.caller.GetWithFullUrl(
      this.apiUrl +
        '/Product/getProductLookupsForCoupons/' +
        couponId +
        '/' +
        type,
    );
  ...
  getUserLookupsForPackage(pid: number, type: number) {
    return this.caller.GetWithFullUrl(
      this.apiUrl + '/User/getUserLookupsForPackage/' + pid + '/' + type,
    );
  }
}
