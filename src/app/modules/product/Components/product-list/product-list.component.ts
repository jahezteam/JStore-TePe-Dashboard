...
export class ProductListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.paginator.searchText =
      this.route.snapshot.paramMap.get('searchValue')!;
    this.searchVal =
      this.paginator.searchText == '*' ? '' : this.paginator.searchText;
    this.searchInput();
  }
  isAuth(per: string) {
  ...
  routeToEdit(id: number) {
    this.router.navigateByUrl(
      '/product/edit/' + id + '/' + this.paginator.searchText,
    );
  }
  ...
  openFilterPopup() {
    ...
    this.ref.onClose.subscribe((data: filter) => {
      if (data != null) {
        ...
        this.service
          .getPaginationFilter(this.paginator, this.filter)
          .subscribe((res: any) => {
            if (res) {
  ...
  searchInput(event: any = null) {
    ...
    this.paginator.pageNumber = 1;
    this.paginator.pageSize = environment.pageSize;
    if (event != null) this.paginator.searchText = event.target.value;
    this.paginator.sortingColumn = 'code';
    this.paginator.sortingType = 'asc';
