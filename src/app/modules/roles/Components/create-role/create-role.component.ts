...
export class CreateRoleComponent implements OnInit , OnDestroy {
  ...
  onseletedChanged(item:string,value:any){
    if(value.checked)
    {
    if(this.selectedPermissions.indexOf(this.permissions.find((x:dropdown)=>x.name==item))==-1)
        this.selectedPermissions.push(this.permissions.find((x:dropdown)=>x.name==item));
    else{
...
this.deleteItem(item);
  ...
  deleteItem(item:string){
    ...
    let index=this.selectedPermissions.indexOf(this.permissions.find((x:dropdown)=>x.name==item));
    if(index!=-1)
