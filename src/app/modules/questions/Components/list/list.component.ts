...
export class ListComponent {
  ...
  loadData() {
    this.service
      .getQuestionTypePagination(this.paginator)
      .subscribe((res: any) => {
        if (res) {
          this.date = res.items;
  ...
  routeToAdd() {
    this.router.navigateByUrl('/questionType/add');
  }
  routeToEdit(id: any) {
    this.router.navigateByUrl('/questionType/edit/' + id);
  }
  routeToDetails(id: any) {
    this.router.navigateByUrl('/questionType/details/' + id);
  }

  routeToQuestions(id: any) {
    this.router.navigateByUrl('/questionType/questions/' + id);
  }
  ...
  openAddPopup() {
    ...
    this.ref.onClose.subscribe((item: QuestionType) => {
      if (item != null) {
        this.service.postQuestionType(item).subscribe((res: any) => {
          if (res) {
            this.messageService.add({
              key: 'tl',
              severity: 'success',
              summary: 'success',
              detail: 'Question Type Created Succesfully',
            });
            this.ngOnInit();
          } else {
            this.messageService.add({
  ...
  openEditPopup(item: QuestionType) {
    this.editRef = this.dialogService.open(UpdateComponent, {
      header: 'Edit Question Type',
      width: '50%',
      contentStyle: { 'max-height': '550px', overflow: 'auto' },
      baseZIndex: 10000,
      data: item,
    ...
    this.editRef.onClose.subscribe((item: QuestionType) => {
      if (item != null) {
        this.service.updateQuestionType(item).subscribe((res: any) => {
          if (res) {
            this.messageService.add({
              key: 'tl',
              severity: 'success',
              summary: 'success',
              detail: 'Question Type updated Succesfully',
            });
            this.ngOnInit();
          } else {
            this.messageService.add({
  ...
  openDetailsPopup(item: QuestionType) {
    this.refDetails = this.dialogService.open(DetailsComponent, {
      header: 'QuestionType Details',
      width: '50%',
      contentStyle: { 'max-height': '550px', overflow: 'auto' },
      baseZIndex: 10000,
      data: item,
    });
    this.refDetails.onClose.subscribe((item: QuestionType) => {
      if (item != null) {
      }
    });
  }
  openFilterPopup() {
    ...
    this.reffilter.onClose.subscribe((data: filter) => {
      if (data != null) {
        ...
        this.service
          .getQuestionTypePaginationFilter(this.filter.isActive, this.paginator)
          .subscribe((res: any) => {
            if (res) {
  ...
  searchInput(event: any) {
    ...
    this.paginator.pageNumber = 1;
    this.service
      .searchQuestionType(this.paginator, true)
      .subscribe((res: any) => {
        if (res) {
          this.date = res.items;
          this.filteredDate = this.date;
  ...
  deleteItem(item: QuestionType) {
    this.confirmationService.confirm({
      message: 'Are you sure you want to Delete This QuestionType',
      header: 'Confirm',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.service.deleteQuestionType(item.id).subscribe((res: any) => {
          if (res) {
            this.messageService.add({
              key: 'tl',
              severity: 'success',
              summary: 'success',
              detail: 'QuestionType Deleted Succesfully',
            });
            this.ngOnInit();
          } else {
            this.messageService.add({
  ...
  protected readonly QuestionsPermissions = QuestionsPermissions;
}
