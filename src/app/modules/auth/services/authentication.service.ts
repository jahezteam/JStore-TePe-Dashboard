...
import { changePassword, resetPassword } from '../../users/Models/changePassword';
...
export class AuthenticationService {
  ...
  changePassword(model: changePassword): Observable<any> {
    this.isLoadingSubject.next(true);
    return this.http.post<any>(`${API_USERS_URL}/ChangePassword`, model, { headers: new HttpHeaders({ 'Authorization': 'Bearer ' + localStorage.getItem('token') }) });
  }
  resetPassword(model: resetPassword): Observable<any> {
    this.isLoadingSubject.next(true);
    return this.http.post<any>(`${API_USERS_URL}/ResetPassword`, model, { headers: new HttpHeaders({ 'Authorization': 'Bearer ' + localStorage.getItem('token') }) });
  }
  getToken() {
  ...
  loadUserData(): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      this.http
        .get(environment.apiUrl + "/User/CurrentUser", { headers: new HttpHeaders({ 'Authorization': 'Bearer ' + localStorage.getItem('token') }) })
        .subscribe((Response) => {
  ...
  ForgetPassword(model: any) {
    return this.http
      .post(environment.apiUrl + "/Account/ForgetPassword", model, { headers: new HttpHeaders({ 'Authorization': 'Bearer ' + localStorage.getItem('token') }) });
  }
  LogoutOthersEmail(model: any) {
    return this.http
      .post(environment.apiUrl + "/Account/SignOutFromSessionsEmail", model, { headers: new HttpHeaders({ 'Authorization': 'Bearer ' + localStorage.getItem('token') }) });
  }
  sendOtp(model: any) {
    return this.http
      .post(environment.apiUrl + "/Account/SendOtp", model, { headers: new HttpHeaders({ 'Authorization': 'Bearer ' + localStorage.getItem('token') }) });
  }
